$(function(){
    ...
    var data = $.getJSON(endPoint, function(result){
        ...
        for(i = 0; i < result.results.length; i++) {
            ...
            $("#my-list-content-con").append("<div class='ml-poster-con col-12 col-lg-6 set'><div class='ml-movie-poster-container'><div class='poster-ratio'><img id='coming-later-poster-1' src='https://image.tmdb.org/t/p/w500/"+poster+"' class='poster-ratio-image-container'></div></div><div class='movie-info-container'><h1 class='poster-title-x'>"+title+"</h1><p class='genre-text-display'>"+genreName+"</p><p class='release-date-display'>Release date: <b><span id='list-release-date'>"+releaseDate+"</span></p></b><p class='synopsis-text'>"+description+"</p><button class='remove-from-list-button'>Remove</button></div></div>")    
            };
        };
